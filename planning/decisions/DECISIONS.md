...
**Decision:** Stdlib `python3 -m venv` invoked via `subprocess.run([sys.executable, "-m", "venv", str(venv)], check=False)`, where `venv` is the FINAL path `<home>/.concierge/tools-venv/` — direct creation, no `.tmp` staging. The `--system-site-packages` flag is NOT passed; stdlib's default is no system-site-packages (isolation), which is exactly what Option 3 requires. The flag exists only as an opt-IN to system-site-packages (`--system-site-packages` with no value, `store_true` semantics); there is no `--system-site-packages=False` form. On subprocess failure (returncode != 0), the helper `shutil.rmtree`'s any partial venv state at the canonical path before raising `ConciergeVenvError` so the operator's next retry starts clean. Hard-crash mid-creation (SIGKILL etc.) leaves partial state on disk; the existing "incomplete venv — remove manually" guard handles that recovery path.
...
Isolation is the *whole point* of Option 3 — the operator may have system-installed torch/numpy/sentence-transformers that conflict with the pins Concierge tools need; the venv must NOT see those. The Day 5 torch failures triage (`SESSION-2026-04-26-01.md` §Appendix C) is a live example of why version drift between two co-resident venvs matters; Option 3's tools-venv must be a clean third venv that imports nothing from system or from the operator's own venvs.

Mechanism for asserting isolation: the regression-guard test asserts `--system-site-packages` is NOT in argv. A future contributor who tries to opt INTO system-site-packages (e.g. to share torch/numpy with the operator's environment) trips this test, forcing them to revisit this entry. The invariant (isolation) is enforced by the absence of the opt-in flag, not by the presence of an explicit-False flag.
...
**Post-hoc correction note:** This decision was originally drafted with `--system-site-packages=False` as the explicit form AND with an atomic `<venv>.tmp` → `<venv>` rename for partial-state protection. Day 6 Step 5 integration test (real venv CLI invocation, no mocks) caught two issues in succession:

1. **Flag form:** the CLI rejects `--system-site-packages=False` — the flag is `store_true`, not `key=value`. Reverted to "no flag passed; stdlib default is isolation."
2. **Atomic rename:** the `.tmp` → final rename was incompatible with Python venv's hard-coded absolute shebang paths in `bin/` scripts and the `home=` field in `pyvenv.cfg`. After rename, `bin/pip`'s shebang still pointed to the `.tmp` path that no longer existed, causing "cannot execute: required file not found" on the first install attempt. Reverted to direct-create-at-final-path per standard Python venv usage; subprocess-failure cleanup via `shutil.rmtree`; hard-crash recovery via existing incomplete-venv guard. Decision corrected post-hoc; isolation invariant still holds.

Both corrections rooted in the same gap: unit tests mocked `subprocess.run` and never exercised the real venv CLI, so they couldn't see CLI-form rejection or shebang-baking semantics. **The wiring-test meta-lesson now has four data points across two days: Day 5's telemetry-commit rollback (`SESSION-2026-04-26-01.md` Task 0), Day 5's SSE wire-format JSON-vs-Python-repr (Task 1), Day 6 Step 5's venv flag form (this entry, correction 1), Day 6 Step 5's venv rename incompatibility (this entry, correction 2).** Strengthens the case that mock-based unit tests are insufficient for any contract that touches OS-level reality (CLI invocations, filesystem semantics, network, persistent state). Future test-strategy choices should default to "if the contract is operator-observable, the wiring test must exercise it against reality, not mocks."
...
**Affects:** New `_ensure_concierge_venv` helper in `core/install/venv.py`. Helper invokes `subprocess.run([sys.executable, "-m", "venv", str(venv)], check=False)` where `venv` is the canonical final path `<home>/.concierge/tools-venv/`. On subprocess failure, `shutil.rmtree(venv, ignore_errors=True)` cleans up partial state before raising `ConciergeVenvError`. `tests/test_venv_bootstrap.py` mocks `subprocess.run` and asserts: (a) the call shape does NOT carry `--system-site-packages` — a future contributor who tries to opt INTO system-site-packages (e.g. to share torch/numpy with the operator's environment) trips this test; (b) subprocess-failure cleans up partial venv state at the canonical path. A separate Day 6 Step 5 integration test (`@pytest.mark.slow`, `TestRealVenvBootstrap`) exercises real venv creation + real pip install + real shim invocation against the actual stdlib CLI, closing the unit-test mock-vs-reality gap. Python-version pinning is implicit (no rebuild logic exists); future "auto-rebuild on Python upgrade" work would need its own DECISIONS entry.
